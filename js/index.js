...
submitTweet.addEventListener("click", function () {
  ...
  if (tweetObj.length === 0) {
    ...
    index = tweetObj[tweetObj.length - 1].index + 1;
  }

  let tweet = addTweetFromInput.value;
  if (tweet !== "" && tweet.trim() !== 0) {
...
function showTweet() {
  ...
  tweetObj.forEach(({ index, tweet, time }) => {
    html += `
      <tr>
...
function editTweet(index) {
  ...
  addTweetFromInput.value = tweetObj[index].tweet;
  console.log(addTweetFromInput.value);
...
saveTweetBtn.addEventListener("click", function () {
  ...
  let webTweet = localStorage.getItem("localTweet");
  let tweetObj = JSON.parse(webTweet);
  tweetObj[saveindex].tweet = addTweetFromInput.value;
  localStorage.setItem("localTweet", JSON.stringify(tweetObj));
  addTweetFromInput.value = "";
